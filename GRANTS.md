...
catalog was last verified against package version `2.5.0` and manifest protocol `2.0`.

Serialized capability names are sourced from the authoritative `CapabilityCatalog` in
`CSweet.Agent.SDK` 3.9.0; manifest-audit tests reject names missing from that catalog.
